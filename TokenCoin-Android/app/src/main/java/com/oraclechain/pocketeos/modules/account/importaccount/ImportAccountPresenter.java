...
public class ImportAccountPresenter extends BasePresent<ImportAccountView> {
    ...
    public void getAccountInfoData(String accountname) {
        ...
        HttpUtils.postRequest(BaseUrl.HTTP_get_chain_account_info, mContext, json, new JsonCallback<ResponseBean<BlockChainAccountInfoBean.DataBean>>() {
            @Override
    ...
    public void postEosAccountData(String eosAccountName, String uid) {
        ...
        String json = new JSONObject(hashMap).toString();
        HttpUtils.postRequest(BaseUrl.HTTP_add_new_eos, mContext, json, new JsonCallback<ResponseBean<String>>() {
            @Override
    ...
    public void setMianAccountData(String eosAccountName) {//0代表直接执行设置主账号操作，1代表先删除后设置主账号
        ...
        hashMap.put("eosAccountName", eosAccountName);
        String json = new JSONObject(hashMap).toString();
        HttpUtils.postRequest(BaseUrl.HTTP_set_mian_account, mContext, json, new JsonCallback<ResponseBean<String>>() {
            @Override
            public void onSuccess(Response<ResponseBean<String>> response) {
                if (response.body().code == 0) {
