...
import com.oraclechain.pocketeos.blockchain.bean.PushSuccessBean2;
...
public class PairDataManger {
    ...
    static String CACTUSCONTRACT  = Constants.CACTUSCONTRACT;
    static String ACTIONBUY       = Constants.ACTIONBUY;
    static String ACTIONSELL      = Constants.ACTIONSELL;
    ...
    public PairDataManger(Context context, String password) {
        mContext = context;
    ...
    public void pushAction(String message, String action, String permissionAccount) {
        this.message = message;
        if (action.equals("buy"))
            this.action = ACTIONBUY;
        else 
            this.action = ACTIONSELL;
        this.contract = CACTUSCONTRACT;
    ...
    public void pushTransactionRetJson(PackedTransaction body) {
        HttpUtils.postRequest(BaseUrl.HTTP_push_transaction, this, mGson.toJson(body), new JsonCallback<ResponseBean<PushSuccessBean2.DataBeanX>>() {
            @Override
            public void onSuccess(final Response<ResponseBean<PushSuccessBean2.DataBeanX>> response) {
                if (ShowDialog.dialog != null) {
                    ShowDialog.dissmiss();
                }
                if (response.body().code == 0) {
                    final Bundle bundle = new Bundle();
                        ToastUtils.showLongToast(R.string.gosuccess);
