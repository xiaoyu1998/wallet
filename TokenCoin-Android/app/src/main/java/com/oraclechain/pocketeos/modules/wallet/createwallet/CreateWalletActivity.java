...
public class CreateWalletActivity extends BaseAcitvity<NormalView, NormalPresenter> implements NormalView {
    ...
    public void onViewClicked() {
        if (TextUtils.isEmpty(mPassword.getText().toString()) || TextUtils.isEmpty(mConfirmPassword.getText().toString())) {
            ...
            UserBean userBean;
            String firstUser = Utils.getSpUtils().getString("firstUser");
            if (firstUser == null) {
                userBean = new UserBean();
                userBean.setWallet_phone(getString(R.string.default_phone));
                userBean.setWallet_name(getString(R.string.my_wallet));
                userBean.setWallet_uid("d553e2e94b9b888341fe1d572b6720b5");
                MyApplication.getDaoInstant().getUserBeanDao().insert(userBean);
                MyApplication.getInstance().setUserBean(userBean);
                Utils.getSpUtils().put("firstUser", getString(R.string.default_phone));//保存上次登陆钱包
                Utils.getSpUtils().put("loginmode", "phone");//保存当前登录模式
            }else{
                userBean = MyApplication.getDaoInstant().getUserBeanDao().queryBuilder().where(UserBeanDao.Properties.Wallet_phone.eq(firstUser)).build().unique();
                String randomString = EncryptUtil.getRandomString(32);
                userBean.setWallet_shapwd(PasswordToKeyUtils.shaEncrypt(randomString+mPassword.getText().toString().trim()));
                MyApplication.getDaoInstant().getUserBeanDao().update(userBean);
                MyApplication.getInstance().getUserBean().setWallet_shapwd(PasswordToKeyUtils.shaEncrypt(randomString+mPassword.getText().toString().trim()));
