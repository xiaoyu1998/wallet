...
public class HomeFragment extends BaseFragment<HomeView, HomePresenter> implements HomeView, MyScrollview.ScrollViewListener {
    ...
    protected void initData() {
        ...
        mCoinAdapter = AdapterManger.getCoinAdapter2(getActivity(), mAccountWithCoinBeen);
        mCoinAdapter.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                AccountWithCoinBean2 coin = mAccountWithCoinBeen.get(position);
                if(coin.getContract().equals("eosio.token")){
...
//        });
    ...
    public void getAccountDetailsDataHttp(List<AccountWithCoinBean2> accountWithCoinBeens) {
        ...
        if (accountWithCoinBeens.size() != 0) {
            ...
            for(int i = 0; i < accountWithCoinBeens.size(); i++ ){
                  BigDecimal toCny =  BigDecimal.valueOf(Double.parseDouble(accountWithCoinBeens.get(i).getCoinForCny()));
                  sum = BigDecimalUtil.add(sum, toCny);
            }
            openMoney = "≈" + StringUtils.addComma(sum + "");
