...
@property(nonatomic, strong) TransactionRecordsService2 *transactionRecordsService;
...
- (TransactionRecordsService2 *)transactionRecordsService{
    if (!_transactionRecordsService) {
        _transactionRecordsService = [[TransactionRecordsService2 alloc] init];
    }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    self.transactionRecordsService.getTransactionRecordsRequest.contract = self.model.contract;
    [self loadNewData];
}
...
- (void)configHeaderView{
    ...
    self.headerView.amountLabel.text = [NSString stringWithFormat:@"%@ %@", [NumberFormatter displayStringFromNumber:@(self.model.assests_balance.doubleValue)],self.model.assestsName];
    ...
    self.headerView.totalLabel.text = [NSString stringWithFormat:@"额(24h)%@CNY",  [NumberFormatter displayStringFromNumber:@((int)(self.model.assests_balance.doubleValue * self.model.assests_price_cny.doubleValue))]];
    ...
    WS(weakSelf);
    self.getSparklinesRequest.currentAssestsType = self.model.assestsName;
    [self.getSparklinesRequest postOuterDataSuccess:^(id DAO, id data) {
        NSString *tendencyUrlStr = data[@"data"][@"sparkline_token_png"];
//        if ([weakSelf.model.assestsName isEqualToString:@"EOS"]) {
//            tendencyUrlStr = data[@"data"][@"sparkline_token_png"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TransactionRecord2 *model = self.transactionRecordsService.dataSourceArray[indexPath.row];
//    if ([self.model.assestsName isEqualToString:@"eos"]) {
...
//    }
    model = self.transactionRecordsService.tokenTransactionDatasourceArray[indexPath.row];
...
//    }
    return self.transactionRecordsService.tokenTransactionDatasourceArray.count;
    return 0;
}
...
- (void)loadNewData
{
    ...
    self.transactionRecordsService.currentSymbolName = _model.assestsName;
