...
//   }
    ...
    int value = 0;
    for( NSDictionary *asset_d in model.account_assets ) {

        NSString *asset_str = [asset_d objectForKey:@"asset"];
        NSArray *aArray = [asset_str componentsSeparatedByString:@" "];

        value += [aArray[0] floatValue] * 50;
    }

    self.totalAssetsLabel.text = [NSString stringWithFormat:@"≈%@", [NumberFormatter displayStringFromNumber:[NSNumber numberWithDouble:value]]];
    //[self.totalAssestsVisibleBtn setImage:[UIImage imageNamed:@"eye_open"] forState:(UIControlStateNormal)];
