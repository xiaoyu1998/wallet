...
- (AssestsMainHeaderView *)headerView{
    if (!_headerView) {
        ...
        _headerView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 248);
...
- (void)viewDidLoad {
    ...
    for (AccountInfo *model in accountArray) {
        if ([model.is_main_account isEqualToString:@"0"]) {
...
- (void)buidDataSource{
    WS(weakSelf);
    self.mainService.getAccountAssetRequest.account_name = self.currentAccountName;
    [self.mainService get_account_asset:^(AccountResult2 *result, BOOL isSuccess) {
        // 拿到当前的下拉刷新控件，结束刷新状态
